...
internal sealed class GradientStartColorCustomization : Customization
{
	...
	public bool RenderImGui(string? parentName = "", GradientStartColorCustomization? defaultCustomization = null)
	{
		...
		if(ImGuiHelper.ResettableTreeNode(localization.Start, customizationName, ref isChanged, defaultCustomization, this.Reset))
		{
			...
			{
				var isStart1Changed = ImGuiHelper.ResettableColorPicker4($"##{customizationName}-1", ref this.ColorInfo1, defaultCustomization?.ColorInfo1);
				isChanged |= isStart1Changed;
