...
{
	private static readonly Lazy<ScreenManager> _lazy = new(() => new ScreenManager());
	...
	public static ScreenManager Instance => _lazy.Value;
	...
	public Vector2? ConvertWorldPositionToScreenPosition(Vector3 worldPosition)
	{
		try
		{
			...
			if(Vector3.Dot(cameraToWorld, -this._cameraForward) <= 0f) return null;
			...
			if(screenX < -this._overheadX) return null;

			if(screenX > this.WindowSize.X + this._overheadX) return null;

			if(screenY < -this._overheadY) return null;

			if(screenY > this.WindowSize.Y + this._overheadY) return null;
