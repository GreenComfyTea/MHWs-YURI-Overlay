		...
		where T : class
	{
		...
		foreach(var property in properties)
		{
			...
			else
			{
				...
				else if(sourceValue is not null && destinationValue is not null)
				{
					...
					typeof(ConfigManager).GetMethod(nameof(Merge))?.MakeGenericMethod(property.PropertyType).Invoke(null, new[] { sourceValue, destinationValue });
				}
			}
		}
		...
		foreach(var field in fields)
		{
			...
			else
			{
				...
				{
					typeof(ConfigManager).GetMethod(nameof(Merge))?.MakeGenericMethod(field.FieldType).Invoke(null, new[] { sourceValue, destinationValue });
				}
			}
		}
